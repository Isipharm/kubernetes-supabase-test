...
topologies = 
  if System.get_env("KUBERNETES_SERVICE_HOST") do
    # Stratégie Kubernetes officielle pour StatefulSet avec DNS SRV
    kubernetes = [
      strategy: Cluster.Strategy.Kubernetes.DNSSRV,
      config: [
        mode: :hostname,
        service: "supabase-supavisor-headless",  # Doit matcher EXACTEMENT le nom du service headless
        namespace: System.get_env("POD_NAMESPACE") || "supabase",
        application_name: System.get_env("NODE_NAME") || "supabase-supavisor",  # Doit matcher le préfixe du nom de nœud (avant le @)
        polling_interval: 5_000
      ]
    ]

    Keyword.put(topologies, :kubernetes, kubernetes)
  else
    topologies
  end

# Supprime TOUTES les autres stratégies de clustering (conflits possibles)
topologies = 
  topologies
  |> Keyword.delete(:dns_poll)
  |> Keyword.delete(:epmd)
  |> Keyword.delete(:postgres)

config :libcluster,
  debug: true,  # Active les logs détaillés pour le debugging
  topologies: topologies

IO.puts("===> runtime.exs injecté et exécuté <===")
IO.inspect(System.get_env("RELEASE_COOKIE"), label: "RELEASE_COOKIE")
IO.inspect(System.get_env("RELEASE_NODE"), label: "RELEASE_NODE")
IO.inspect(System.get_env("NODE_IP"), label: "NODE_IP")
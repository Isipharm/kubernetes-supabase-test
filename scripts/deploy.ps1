...
k3d cluster create supabase-cluster
...
Write-Host "kubectl port-forward svc/argocd-server -n argocd 8888:443" -ForegroundColor Cyan
...
if ($encodedPassword) {
    ...
    Write-Host "kubectl -n argocd get secret argocd-initial-admin-secret -o jsonpath=`"{.data.password}`" | [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String(`$(`$_)))" -ForegroundColor DarkGray
}
